...
#     bash scripts/mindts_anomaly/UniTS_zeroshot_mindts_anomaly.sh

ckpt_path=${CKPT:-}
model_name=${MODEL_NAME:-UniTS_zeroshot}
exp_name=${EXP_NAME:-zeroshot_mindts_anomaly}
project_name=${PROJECT_NAME:-mindts_anomaly_detection}
result_root=${RESULT_ROOT:-results}
result_tag=${RESULT_TAG:-mindts_anomaly}
...
read -r -a anomaly_ratios <<< "${ANOMALY_RATIOS:-1}"
