...
class TimeMMDForecastDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = df_raw.dropna(subset=['date', self.target]).sort_values('date').reset_index(drop=True)
        df_raw[self.target] = df_raw[self.target].interpolate().ffill().bfill()
...
class MindTSSegLoader(Dataset):
    ...
    @staticmethod
    def read_otb_csv(path):
        raw = pd.read_csv(path)
        if "cols" not in raw.columns:
            raise ValueError(f"MindTS file must contain a 'cols' column: {path}")
    ...
    def _resolve_paths(self):
        ...
        flat_data_path = root / csv_name
        flat_meta_path = root / "DETECT_META.csv"
        nested_data_path = root / "dataset" / "anomaly_detect" / "data" / csv_name
        nested_meta_path = root / "dataset" / "anomaly_detect" / "DETECT_META.csv"

        if flat_data_path.exists() and flat_meta_path.exists():
            return flat_data_path, flat_meta_path
        return nested_data_path, nested_meta_path

    def __read_data__(self):
        data_path, meta_path = self._resolve_paths()
        ...
        df = self.read_otb_csv(data_path)
        meta = pd.read_csv(meta_path).set_index("file_name")
        train_len = int(meta.loc[data_path.name, "train_lens"])
        ...
        self.train = train_data
        self.test = test_data
        self.train_labels = labels[:train_len].reshape(-1, 1)
        self.test_labels = labels[train_len:].reshape(-1, 1)
        data_len = len(self.train)
        self.val = self.train[int(data_len * 0.8):]
    ...
    def __len__(self):
        ...
        return max((length - self.win_size) // self.step + 1, 0)
...
class UEAloader(Dataset):
    ...
    def load_single(self, filepath):
        if load_from_tsfile_to_dataframe is None:
            raise ImportError("sktime is required for UEAloader but is not installed.")
...
class GLUONTSDataset(Dataset):
    ...
    def __init__(self,
        ...
        if get_dataset is None:
            raise ImportError("gluonts is required for GLUONTSDataset but is not installed.")
